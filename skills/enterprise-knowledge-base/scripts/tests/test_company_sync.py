...
import sync_company_knowledge as sync
...
class CompanySyncTests(unittest.TestCase):
    ...
    def test_failed_first_sync_rolls_back_employee_completion_and_writes_no_mirror(self) -> None:
        class BrokenReader(FakeKnowledgeReader):
            def list_tree(self, space_id: str):
                raise sync.CompanySyncError("read failed")
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            employee = root / "employee"
            evidence = prepare_employee(admin, employee)
            with self.assertRaises(sync.CompanySyncError):
    ...
    def test_session_first_enterprise_query_syncs_once_and_personal_never_syncs(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            admin = configure_admin(root / "admin")
            employee = configure_employee(admin, root / "employee")
            reader = FakeKnowledgeReader()
            personal = coordinator.before_company_query(employee, "personal", "task-1", reader)
            self.assertEqual(personal["remote_reads"], 0)
            first = coordinator.before_company_query(employee, "enterprise", "task-1", reader)
            second = coordinator.before_company_query(employee, "enterprise", "task-1", reader)
            third = coordinator.before_company_query(employee, "enterprise", "task-2", reader)
